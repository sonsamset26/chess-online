...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccessLogin: (userData: { id?: string; username: string; eloRating: number; avatarUrl?: string; token: string }) => void;
}
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-in fade-in duration-200 select-none">
      <div className="w-full max-w-md bg-[#16202E] border border-[#243247] rounded-2xl p-6 shadow-2xl relative">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-[10px] text-[#94A3B8] font-semibold uppercase">Hoặc dùng Mật khẩu</span>
          <div className="flex-1 h-[1px] bg-[#334155]" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {!isLogin && (
            <div>
              <label className="text-xs font-bold text-[#E2E8F0] mb-1 block">Tên hiển thị:</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-[#1E293B] border border-[#334155] text-white text-sm focus:outline-none focus:border-pink-500 transition-colors"
                />
              </div>
            </div>
          )}
          ...
          <div>
            <label className="text-xs font-bold text-[#E2E8F0] mb-1 block">Địa chỉ Email:</label>
            <div className="relative">
          ...
          <div>
            <label className="text-xs font-bold text-[#E2E8F0] mb-1 block">Mật khẩu:</label>
            <div className="relative">
